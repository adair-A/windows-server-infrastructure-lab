...
I performed these checks during the original lab walkthrough.

| Capability | Validation method | Result |
| --- | --- | --- |
...
| OU-scoped Group Policy | Attempted restricted tools as the restricted user | Registry Editor displayed a message that editing was disabled by the administrator; I tested other restrictions during the walkthrough |
...
| DNS | Used a hostname-based ping during client command-line testing | The hostname resolved and returned replies during my test |
...
The IIS site did not load reliably during one client attempt, but I later loaded the page from Server B. I could not determine why the client attempt failed.
